...
from testApp.schemas.user.user import UserLogin
...
from testApp.schemas.token.token import TokenInfo
...
def check_jwt(request: Request):
    ...
    try:
        payload = jwt.decode(token, settings.auth_jwt.public_key_path.read_text(), algorithms=settings.auth_jwt.algorithm)
        return payload  
...
async def login_user(schema: UserLogin,
    user_service: Annotated[UserService,Depends(user_service)],
    response: Response,
    session: AsyncSession = Depends(db_helper.get_session)
    ):
    result = await user_service.login_user(schema,session)
    response.set_cookie(key="access_token", value=result, httponly=True, secure=True, samesite="Lax")
    return TokenInfo(
        access_token=result,
        token_type="Bearer",
    )
